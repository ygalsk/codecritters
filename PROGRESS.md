...
- **README rewrite** (`README.md`): Game-first tone — leads with pitch and install, not tech docs. Sections: Play It (download + build from source), What You Do (core loop), The World (7 biomes table with triggers and flavor), The Critters (highlights table), The Type Chart (full 7×7 with narrative one-liners), Your Code Fuels the Game (Claude Code integration + CLI reference), Controls (per-screen keybinding table). Developer section below the fold: build/test commands, project layout, game data architecture, contributing guide with sprite specs.
- **Install script** (`install.sh`): `curl | sh` installer. Detects arch (x86_64/aarch64) and OS (Linux/macOS). Downloads latest release tarball from GitHub, extracts to `~/.local/share/codecritter/`, creates wrapper script in `~/.local/bin/` that `cd`s to install dir before exec (needed because game loads `data/` and `assets/` relative to CWD). Configurable via `CODECRITTER_HOME` and `CODECRITTER_BIN` env vars.
- **GitHub Actions release workflow** (`.github/workflows/release.yml`): Triggered on `v*` tags. Matrix: Linux x86_64 (ubuntu-latest) + Linux aarch64 (ubuntu-24.04-arm). Uses `mlugg/setup-zig@v2` for Zig 0.15.2. Builds ReleaseSafe, bundles binary + data/ + assets/ + LICENSE into tarball, uploads to GitHub Release via `softprops/action-gh-release@v2`.
- **MIT License** (`LICENSE`): Standard MIT license added.
- **build.zig.zon**: Uncommented `LICENSE` and `README.md` in `.paths` for package inclusion.
- **Data architecture decision**: Evaluated JSON splitting — kept monolithic files. At 52KB total (species 19KB/61 entries, moves 7KB/50 entries, items 3KB/11 entries, biomes 23KB/7 entries), splitting adds directory enumeration complexity for zero performance or ergonomic benefit. Rationale documented in README contributing section.
- **Screenshot placeholders**: `assets/screenshots/.gitkeep` created for future captures. README has TODO comment for hero image.
- **Files changed**: `README.md`, `LICENSE`, `install.sh`, `.github/workflows/release.yml`, `build.zig.zon`, `assets/screenshots/.gitkeep`, `AGENT_INSTRUCTIONS.md`, `PROGRESS.md`

## Phase 29 — Balance Pass + Final Polish [DONE]
- **Epic stat rebalance** (`data/species.json`): Bumped all 7 epic critters from 360-395 total to 415-440 range, eliminating overlap with rare tier (340-365). Preserved intentional extreme spreads (dump stats) while adding budget to non-dump stats. Valgrind 390→420, Race Condition 390→425, Load Balancer 390→420, Turing Machine 360→415, Regex 390→430, Prompt Engineer 390→415, Mainframe 395→440.
- **Legendary stat bump** (`data/species.json`): All 3 legendaries bumped from 440-450 to 470-475 range. Root 450→475, Zero Day 450→475, Linus 440→470. Clear daylight above epic tier.
- **Catch rate fix** (`src/battle/catch.zig`): Changed Formal Proof type bonus vs chaos from -30 (penalty) to +10 (modest bonus). Formal verification thematically counters chaos. Updated test to match.
- **Balance verification**: Moves (50 moves, 4 power tiers with inverse accuracy), XP curve (quadratic, appropriate for roguelike), shop pricing (10%/floor scaling), biome encounters (type-focused with floor gating) — all verified as well-balanced, no changes needed.
- **XP grant consumable items** (`src/data/items.zig`, `data/items.json`): New `xp_grant` ItemKind + `xp_amount: ?u32` field. Three items: Code Review (150 XP, $200, uncommon), Tech Talk (500 XP, $500, rare), Pair Programming (1500 XP, $1200, epic). Calibrated to leveling curve: ~1 level at low/mid/high levels respectively.
- **Inventory XP use** (`src/ui/inventory_screen.zig`): Full integration — handleBrowsing allows target selection for xp_grant items, handleSelectTarget calls `leveling.awardXp()` (handles level-up + evolution), isValidTarget validates alive critter below level 100. "XP Items" category in browsing view with (+XP) annotations. Detail panel shows XP grant amount. Target select shows "Choose a critter to train:" prompt.
- **Shop/drop availability** (`data/biomes.json`): Code Review (weight 10) and Tech Talk (weight 4) added to all 7 biomes' shop_bias. All 3 XP items added to drop_table (Code Review weight 5, Tech Talk weight 2, Pair Programming weight 1 — boss-drop rarity).
- **Animated health bars** (`src/ui/battle_anim.zig`, `src/ui/battle_screen.zig`): Smooth HP drain during battle damage animations. Architecture: `player_display_hp`/`wild_display_hp` fields on AnimState decouple rendered HP from mutated HP. Pre-turn HP snapshot in `submitAction()`. During shake step, `easeOutQuad` tweens display HP from pre-damage to post-damage value over 200ms. `shake_hp_start` tracker ensures correct tween start per step. Display HP preserved across step transitions but fully reset on animation finish/skip, falling back to `critter.current_hp`. `renderCritterInfo` reads display HP from sequencer state.
- **Files changed**: `data/species.json`, `data/items.json`, `data/biomes.json`, `src/data/items.zig`, `src/battle/catch.zig`, `src/dungeon/biome.zig`, `src/ui/inventory_screen.zig`, `src/ui/battle_anim.zig`, `src/ui/battle_screen.zig`, `AGENT_INSTRUCTIONS.md`, `PROGRESS.md`
