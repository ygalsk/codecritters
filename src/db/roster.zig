...
pub fn purchaseMetaUpgrade(database: *Db, upgrade_id: []const u8, cost: u32) bool {
    if (!spendCurrency(database, cost)) return false;
    var key_buf: [64]u8 = undefined;
...
test "meta upgrade purchase" {
    ...
    try std.testing.expectEqual(@as(u32, 850), getCurrency(&db));
    ...
    try std.testing.expectEqual(@as(u32, 550), getCurrency(&db));
